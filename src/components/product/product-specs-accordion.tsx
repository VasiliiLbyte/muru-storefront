...
function SpecsTable({
  ...
  return (
    <dl className={cn("flex flex-col gap-3", className)}>
      {rows.map((row) => (
        <div key={row.label} className="flex items-baseline gap-2 text-body">
 ...
 *
 * Mobile: плоская таблица «ключ → значение», артикул — такая же строка
 * (стилистика kuchenland, аккордеона нет).
 * Desktop: артикул/цвет строками + аккордеон «Характеристики» — как было.
 */
export function ProductSpecsAccordion({
  ...
  return (
    <div className={cn("mt-8 flex flex-col gap-4 border-t border-border pt-6", className)}>
      {/* Mobile: всё плоской таблицей */}
      <SpecsTable rows={mobileRows} className="lg:hidden" />

      {/* Desktop: без изменений */}
      <div className="hidden flex-col gap-4 lg:flex">
        <p className="text-body text-text-secondary">
          Артикул: <span className="text-text-heading">{product.sku}</span>
        </p>
        {colorValue ? (
          <p className="text-body text-text-secondary">
        ...
        {rows.length > 0 ? (
          <details className="group border-t border-border pt-4">
            ...
            <SpecsTable rows={rows} className="mt-4" />
          </details>
        ) : null}
      </div>
    </div>
